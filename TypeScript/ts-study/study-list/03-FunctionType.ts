...
function func03_1(x: number): number {
    return x * 2    // 파라미터 x에 2를 곱하여 return
}

func03_1(6);
// func03_1('x');     // 함수의 파라미터를 number로 지정하여 오류 발생

// return을 사용하지 않는 함수의 경우 void 타입을 지정
function func03_2(x:number): void {
    1 + 1;
}
...
function func03_3(x?: number): void {

}

// Test 1.
const func03_4 = (x?: string) => {
    if(x) console.log(`안녕하세요 ${x}`);
    else console.log("이름이 없습니다");
}

// Test 2.
function func03_5(x: number|string): number {
